...
#from mapDIA_justXML import parseXMLfile
import re
import sys
import csv
import os
...
while j<len(accAr):
    if all(['sp|' in proNames[j], begPos != '#N/A', endPos != '#N/A', cysAr[j] != 'no site']):
        ...
        formatPeptide.append(modSeqs[j])
        formatMz.append(prodMz[j])
        
        formatArea.append(data[j+1][indexArea:])
          
           
        formatProtein.append(proNames[j])
        formatPepSeq.append(pepSeqs[j])
        if retTime != []:
            formatRT.append(retTime[j])
                ...
                if elem1.tag == '{http://regis-web.systemsbiology.net/pepXML}spectrum_query':
                    ...
                    try:
                        if any([hitPeptide=='', protAltAr==[], PrecMz=='', modPep=='', modScores=='', modMassAr==[], modPosAr==[]]):
                            ...
                            resultArray[x]=[hitPeptide, protAltAr, PrecMz, modPep, modScores, modMassAr, modPosAr]
                            x+=1
                    except NameError:
                        donothing=''
...
hits2=[None] * 9000
xml=sys.argv[4]
parseXMLfile(xml, aaMass, hits2)

def remove_values_from_list(the_list, val):
    return [value for value in the_list if value != val]
#filter(lambda mnsAr: mnsAr != None, x)
hits2=remove_values_from_list(hits2, None)
print len(hits2)
###arg 3
ptmProphetScore=sys.argv[3]
i=0
mnsAr=[None] * len(formatProtein)                     
...
def remove_values_from_list(the_list, val):
    return [value for value in the_list if value != val]
###filter(lambda mnsAr: mnsAr != None, x)
mnsAr=remove_values_from_list(mnsAr, None)
print len(mnsAr)
labelAr=[]
...
fnThree = 'mapDIA_Input.txt'
with open(fnThree, 'wb') as myfile:
    outputFile = csv.writer(myfile, delimiter='\t')
    i=0
    outputFile.writerow(labelAr)
    while i<len(mnsAr):
        outputFile.writerows([mnsAr[i]])
        i+=1
